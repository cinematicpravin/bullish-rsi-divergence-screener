...
def find_adjacent_trading_day(d, direction):
    """
    Find the actual nearest NSE trading day, not merely the nearest
    locally cached file.

    This is important on Streamlit Cloud: the local cache may only contain
    older dates. For example, when the selected date is Saturday 15-Aug-2026,
    Previous Trading Day must be 14-Aug-2026 even if 14-Aug has not yet been
    cached in the current Cloud session.
    ...
    # NSE weekends/holidays are detected by checking the actual Bhavcopy
    # archive. A successful download means that date is a trading day.
    session = requests.Session()
    session.headers.update(HEADERS)

    max_calendar_days = 15

    for _ in range(max_calendar_days):
        if direction == 1 and candidate > today:
            return None

        csv_path = DATA_DIR / f"{candidate.strftime('%Y%m%d')}.csv"

        if csv_path.exists():
            return candidate

        ok, _status = download_nse_day(candidate, session)
...
if valid_date:
    ...
    prev_cached = find_adjacent_trading_day(
        selected_date,
        -1
    )

    next_cached = find_adjacent_trading_day(
        selected_date,
        +1
    )
