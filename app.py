...
def _download_url(session, url, timeout=30):
    """Download an NSE archive URL with validation and useful errors."""
    ...
    # NSE occasionally returns an HTML error page with HTTP 200.
    if (
        "text/html" in content_type
        and not response.content.startswith(b"PK")
    ):
        return None, "NSE returned an HTML page instead of market data"

    if len(response.content) < 100:
        return None, "NSE returned an empty/too-small response"
...
def ensure_nse_history(target_date, required_trading_days=80):
    """
    Ensure target date + sufficient preceding NSE trading-day files exist.

    Weekends/known holidays are skipped without making network requests.
    """
    DATA_DIR.mkdir(exist_ok=True)
    ...
    last_status = "No dates checked"

    while found < required_trading_days and checked < max_calendar_days:
        checked += 1

        if not is_nse_equity_trading_day(dt):
            dt -= timedelta(days=1)
            continue
        ...
        if csv_path.exists():
            ...
            if ok:
                ...
                last_status = (
                    f"{dt.strftime('%d-%b-%Y')}: {status}"
                )
            else:
                last_status = (
                    f"{dt.strftime('%d-%b-%Y')}: download failed"
                )
